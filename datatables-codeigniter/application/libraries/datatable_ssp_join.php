...
class Datatable_ssp_join {
    ...
    static function order ( $request, $columns, $isJoin = false )
    {
        ...
        if ( isset($request['order']) && count($request['order']) ) {
            $orderBy = array();
            $dtColumns = Datatable_ssp_join::pluck( $columns, 'dt' );
        ...
        if ( isset($request['search']) && $request['search']['value'] != '' ) {
            ...
            for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
                ...
                if ( $requestColumn['searchable'] == 'true' ) {
                    $binding = Datatable_ssp_join::bind( $bindings, '%'.$str.'%', PDO::PARAM_STR );
    ...
    static function simple ( $request, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '')
    {
        ...
        // CI INTEGRATION DB
        $object = new datatable_ssp_join();
        $sql_details = $object->sql_details();
        // END CI INTEGRATION DB

        $db = Datatable_ssp_join::sql_connect( $sql_details );

        // Build the SQL query string from the request
        $limit = Datatable_ssp_join::limit( $request, $columns );
        $order = Datatable_ssp_join::order( $request, $columns, $joinQuery );
        $where = Datatable_ssp_join::filter( $request, $columns, $bindings, $joinQuery );
        ...
        if($joinQuery){
            $col = Datatable_ssp_join::pluck($columns, 'db', $joinQuery);
            ...
            $query =  "SELECT SQL_CALC_FOUND_ROWS `".implode("`, `", Datatable_ssp_join::pluck($columns, 'db'))."`
			 FROM `$table`
			 $where
			 $extraWhere
			 $groupBy
        ...
        $data = Datatable_ssp_join::sql_exec( $db, $bindings,$query);

        // Data set length after filtering
        $resFilterLength = Datatable_ssp_join::sql_exec( $db,
            "SELECT FOUND_ROWS()"
        );
        $recordsFiltered = $resFilterLength[0][0];

        // Total data set length
        $resTotalLength = Datatable_ssp_join::sql_exec( $db,
        ...
        return array(
            ...
            "data"            => Datatable_ssp_join::data_output( $columns, $data, $joinQuery )
        );
    }
    ...
    static function sql_exec ( $db, $bindings, $sql=null )
    {
        ...
        try {
            ...
            Datatable_ssp_join::fatal( "An SQL error occurred: ".$e->getMessage() );
        }
